import os
import json
import logging
import httpx
from fastapi import FastAPI, Request
from dotenv import load_dotenv

load_dotenv()
app = FastAPI()

DISCORD_WEBHOOK_URL = os.getenv("DISCORD_WEBHOOK_URL")
ETHERSCAN_API_KEY = os.getenv("ETHERSCAN_API_KEY")
ETHERSCAN_API_URL = os.getenv("ETHERSCAN_API_URL", "https://api.etherscan.io/api")

logging.basicConfig(level=logging.INFO)

async def fetch_tx_details_from_etherscan(tx_hash):
    try:
        url = f"{ETHERSCAN_API_URL}?module=proxy&action=eth_getTransactionByHash&txhash={tx_hash}&apikey={ETHERSCAN_API_KEY}"
        async with httpx.AsyncClient() as client:
            response = await client.get(url)
            data = response.json().get("result", {})
            return data
    except Exception as e:
        logging.error(f"[ERROR] Etherscan API failed: {e}")
        return {}

@app.post("/webhook")
async def webhook_listener(request: Request):
    try:
        payload = await request.json()
        logging.info(f"Payload received: {json.dumps(payload)[:500]}...")

        block = payload.get("block", {})
        logs = block.get("logs", [])
        transactions = block.get("transactions", [])
        messages = []

        # Logs-based messages
        for log in logs:
            tx = log.get("transaction", {})
            tx_hash = tx.get("hash", "Unknown")
            from_addr = tx.get("from", {}).get("address", "Unknown")
            to_addr = tx.get("to", {}).get("address", "Unknown")
            value_hex = tx.get("value", "0x0")
            value_eth = int(value_hex, 16) / 1e18 if value_hex else 0
            topics = log.get("topics", [])
            token_transfer = len(topics) > 0 and topics[0].startswith("0xddf252ad")

            msg = f"**New Transaction**
"
            msg += f"**Tx Hash:** [`{tx_hash}`](https://etherscan.io/tx/{tx_hash})
"
            msg += f"**From:** `{from_addr}`
**To:** `{to_addr}`
**Value:** `{value_eth:.6f} ETH`
"
            if token_transfer:
                msg += f"**Type:** Token Transfer
"
            messages.append(msg + "----------------------------")

        # Fallback if logs are empty, but transactions exist
        if not logs and transactions:
            for tx in transactions:
                tx_hash = tx.get("hash")
                if not tx_hash:
                    continue
                details = await fetch_tx_details_from_etherscan(tx_hash)
                msg = f"**Transaction via Etherscan**
"
                msg += f"**Tx Hash:** [`{tx_hash}`](https://etherscan.io/tx/{tx_hash})
"
                msg += f"**From:** `{details.get('from', 'N/A')}`
"
                msg += f"**To:** `{details.get('to', 'N/A')}`
"
                msg += f"**Gas:** `{details.get('gas', 'N/A')}`
"
                msg += f"**Nonce:** `{details.get('nonce', 'N/A')}`
"
                msg += f"**Method (input):** `{details.get('input', '')[:10]}...`
"
                messages.append(msg + "----------------------------")

        if not messages:
            messages.append("No logs or transactions found.")

        async with httpx.AsyncClient() as client:
            for msg in messages:
                await client.post(DISCORD_WEBHOOK_URL, json={"content": msg})

        return {"status": "ok"}
    except Exception as e:
        logging.error(f"[ERROR] Webhook processing failed: {e}")
        return {"status": "error", "details": str(e)}